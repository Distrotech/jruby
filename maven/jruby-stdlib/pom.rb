...
project 'JRuby Stdlib' do
  ...
  execute 'jrubydir', 'prepare-package' do |ctx|
    require File.join( ctx.project.properties[ 'jruby.home' ], 'core/src/main/ruby/jruby/commands.rb' )
    JRuby::Commands.generate_dir_info( ctx.project.build.output_directory.to_pathname )
  end
...
")
		...
		Dir[File.join( \'${jruby.home}\', \'bin\', \'*\' )].each do |fn|
